...
// This file is a copy of $GOROOT/src/go/core/gcimporter/gcimporter_test.go,
// adjusted to make it build with code from (std lib) core/testenv copied.
...
func TestVersionHandling(t *testing.T) {
	...
	for _, f := range list {
		...
		} else if msg := err.Error(); !strings.Contains(msg, "core error") {
...
func TestIssue13898(t *testing.T) {
	...
	// import go/core/gcimporter which imports go/types partially
	imports := make(map[string]*types.Package)
	_, err := gcimporter.Import(imports, "go/core/gcimporter", ".", nil)
	if err != nil {
		t.Fatal(err)
	}
