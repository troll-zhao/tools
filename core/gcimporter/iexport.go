...
func constantToFloat(x constant.Value) *big.Float {
	...
	// (see Mpprec in cmd/compile/core/gc/mpfloat.go).
	const mpprec = 512
	var f big.Float
	f.SetPrec(mpprec)
...
func assert(cond bool) {
	if !cond {
		panic("core error: assertion failed")
	}
}

// The below is copied from go/src/cmd/compile/core/gc/syntax.go.
...
// core inconsistency, whereas the latter is used for ordinary
