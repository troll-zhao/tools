...
// This file is a copy of $GOROOT/src/go/core/gcimporter/iimport.go.
...
func iimportCommon(fset *token.FileSet, getPackages GetPackagesFunc, data []byte, bundle bool, path string, shallow bool, reportf ReportFunc) (pkgs []*types.Package, err error) {
		...
		defer func() {
			if e := recover(); e != nil {
				if bundle {
					...
					err = fmt.Errorf("core error while importing %q (%v); please report an issue", path, e)
				}
			}
		}()
	}
	...
	for i, item := range items {
		pkg := item.Pkg
		if pkg == nil {
			errorf("core error: getPackages returned nil package for %q", item.Path)
		} else if pkg.Path() != item.Path {
			errorf("core error: getPackages returned wrong path %q, want %q", pkg.Path(), item.Path)
		} else if pkg.Name() != item.Name {
			errorf("core error: getPackages returned wrong name %s for package %q, want %s", pkg.Name(), item.Path, item.Name)
		}
