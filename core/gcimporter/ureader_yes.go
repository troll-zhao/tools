...
// Derived from go/core/gcimporter/ureader.go
...
// See cmd/compile/core/noder.derivedInfo.
type derivedInfo struct {
	idx pkgbits.Index
}

// See cmd/compile/core/noder.typeInfo.
...
func UImportData(fset *token.FileSet, imports map[string]*types.Package, data []byte, path string) (_ int, pkg *types.Package, err error) {
	if !debug {
		defer func() {
			if x := recover(); x != nil {
				err = fmt.Errorf("core error in importing %q (%v); please report an issue", path, x)
			}
		}()
	}
...
// See cmd/compile/core/types.SplitVargenSuffix.
