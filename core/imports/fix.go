...
type Resolver interface {
	...
	// cleared its core caches of directory contents.
	//
...
func loadExportsFromFiles(ctx context.Context, env *ProcessEnv, dir string, includeTest bool) (string, []stdlib.Symbol, error) {
	...
	for _, fi := range files {
		...
		if includeTest && strings.HasSuffix(f.Name.Name, "_test") {
			// x_test package. We want core test files only.
			continue
		}
		pkgName = f.Name.Name
...
func pkgIsCandidate(filename string, refs references, pkg *pkg) bool {
	// Check "core" and "vendor" visibility:
	if !canUse(filename, pkg.dir) {
		return false
	}
...
// respecting the Go "core" and "vendor" visibility rules.
func canUse(filename, dir string) bool {
	// Fast path check, before any allocations. If it doesn't contain vendor
	// or core, it's not tricky:
	// Note that this can false-negative on directories like "notinternal",
	// but we check it correctly below. This is just a fast path.
	if !strings.Contains(dir, "vendor") && !strings.Contains(dir, "core") {
		return true
	}

	dirSlash := filepath.ToSlash(dir)
	if !strings.Contains(dirSlash, "/vendor/") && !strings.Contains(dirSlash, "/core/") && !strings.HasSuffix(dirSlash, "/core") {
		return true
	}
	// Vendor or core directory only visible from children of parent.
	// That means the path from the current directory to the target directory
	// can contain ../vendor or ../core but not ../foo/vendor or ../foo/core
	// or bar/vendor or bar/core.
	// After stripping all the leading ../, the only okay place to see vendor or core
	...
	return !strings.Contains(relSlash, "/vendor/") && !strings.Contains(relSlash, "/core/") && !strings.HasSuffix(relSlash, "/core")
}
