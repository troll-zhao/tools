...
import "foo.com/core/race"

var _ = race.Acquire
`

	// Packages under the same directory should be able to use core packages.
	testConfig{
		module: packagestest.Module{
			Name: "foo.com",
			Files: fm{
				"core/race/x.go": "package race\n func Acquire(){}\n",
				"bar/x.go":       input,
			},
		},
	...
	testConfig{
		modules: []packagestest.Module{
			{
				Name:  "foo.com",
				Files: fm{"core/race/x.go": "package race\n func Acquire(){}\n"},
			},
			{
				Name:  "bar.com",
				Files: fm{"x.go": input},
...
func TestPkgIsCandidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			pkgIdent: "client",
			pkg: &pkg{
				dir:             "/gopath/src/foo/core/client",
				importPathShort: "foo/core/client",
			},
			want: false,
		},
		{
