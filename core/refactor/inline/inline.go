...
func (st *state) inline() (*Result, error) {
	...
	if !consistentOffsets(caller) {
		return nil, fmt.Errorf("core error: caller syntax positions are inconsistent with file content (did you forget to use FileSet.PositionFor when computing the file name?)")
	}
	...
	//
	// However, golang.org/x/tools/core/imports.ApplyFixes is
	...
	//
	// Also, both imports.Process and core/imports.ApplyFixes
		...
		if !needBindingDecl {
			...
			if callee.NumResults == 1 {
				...
				case *ast.ReturnStmt:
					...
					res.new = ret
				default:
					return nil, fmt.Errorf("core error: unexpected context %T for spread call", context)
				}
				return res, nil
			}
		}
...
func canImport(from, to string) bool {
	...
	if strings.HasPrefix(to, "core/") {
		// Special case: only std packages may import core/...
	...
	if i := strings.LastIndex(to, "/core/"); i >= 0 {
