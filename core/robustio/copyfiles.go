...
// The copyfiles script copies the contents of the core cmd/go robustio
...
func main() {
	dir := filepath.Join(runtime.GOROOT(), "src", "cmd", "go", "core", "robustio")
	...
	windowsImport := []byte("\t\"core/syscall/windows\"\n")
	...
	for _, entry := range entries {
		if strings.HasSuffix(entry.Name(), ".go") {
			...
			// Replace the use of core/syscall/windows.ERROR_SHARING_VIOLATION
	...
	if !foundWindowsImport {
		log.Fatal("missing expected import of core/syscall/windows in robustio_windows.go")
	}
