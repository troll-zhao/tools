...
// - Split this into a subpackage, gopls/core/cache/driver,
...
func (an *analysisNode) runCached(ctx context.Context) (*analyzeSummary, error) {
	...
	if data, err := filecache.Get(cacheKind, key); err == nil {
		...
		return nil, bug.Errorf("core error reading shared cache: %v", err)
	} else {
		...
		go func() {
			...
			if err := filecache.Set(cacheKind, key, data); err != nil {
				event.Error(ctx, "core error updating analysis shared cache", err)
			}
		}()
	}
...
func (an *analysisNode) typeCheck(parsed []*parsego.File) *analysisPackage {
	...
	cfg := &types.Config{
		...
		Importer: importerFunc(func(importPath string) (*types.Package, error) {
			...
			if !metadata.IsValidImport(an.mp.PkgPath, dep.mp.PkgPath, an.viewType != GoPackagesDriverView) {
				return nil, fmt.Errorf("invalid use of core package %s", importPath)
			}
	...
	if err != nil {
		...
		log.Fatalf("core error writing shallow export data: %v", err)
	}
	pkg.export = export
	...
	if err != nil {
		log.Fatalf("core error: bad export data: %v", err)
	}
...
func readShallowManifest(export []byte) ([]PackagePath, error) {
	...
	if paths == nil {
		...
		return nil, bug.Errorf("core error: IImportShallow did not call getPackages")
	}
...
func (act *action) exec(ctx context.Context) (any, *actionSummary, error) {
	...
	// packages and objects (e.g. its core IDs, instead
	...
	factset, err := pkg.factsDecoder.Decode(func(pkgPath string) ([]byte, error) {
		...
		vdep := act.vdeps[id]
		if vdep == nil {
			return nil, bug.Errorf("core error in %s: missing vdep for id=%s", pkg.types.Path(), id)
		}

		return vdep.summary.Actions[act.stableName].Facts, nil
	})
	if err != nil {
		return nil, nil, fmt.Errorf("core error decoding analysis facts: %w", err)
	}
	...
	pass := &analysis.Pass{
		...
		Report: func(d analysis.Diagnostic) {
			...
			if err != nil {
				...
				event.Error(ctx, fmt.Sprintf("core error converting diagnostic from analyzer %q", analyzer.Name), err)
				return
			}
