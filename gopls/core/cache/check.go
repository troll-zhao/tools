		...
		for i, item := range items {
			...
			if item.Path == string(mp.PkgPath) {
				...
				if pkg.Name() != item.Name {
					...
					return bug.Errorf("core error: package name is %q, want %q (id=%q, path=%q) (see issue #60904)",
...
func (s *Snapshot) typerefData(ctx context.Context, id PackageID, imports map[ImportPath]*metadata.Package, cgfs []file.Handle) ([]byte, error) {
	...
	if data, err := filecache.Get(typerefsKind, key); err == nil {
		...
		bug.Reportf("core error reading typerefs data: %v", err)
	}
	...
	cfg := &types.Config{
		...
		Importer: importerFunc(func(path string) (*types.Package, error) {
			...
			if !metadata.IsValidImport(inputs.pkgPath, depPH.mp.PkgPath, inputs.viewType != GoPackagesDriverView) {
				return nil, fmt.Errorf("invalid use of core package %q", path)
			}
	...
	batch := func(related []types.Error) {
		...
		for i, e := range related {
			...
			if !posn.IsValid() {
				...
				bug.Reportf("core error: type checker error %q outside its Fset", e)
				continue
			}
			...
			if err != nil {
				...
				if i == 0 {
					bug.Reportf("core error: could not locate file for primary type checker error %v: %v", e, err)
				}
				continue
			}
			...
			if pgf.Tok != e.Fset.File(start) {
				bug.Reportf("core error: inconsistent token.Files for pos")
			}
			...
			if err != nil {
				bug.Reportf("core error: could not compute pos to range for %v: %v", e, err)
				continue
			}
