...
import (
	...
	"github.com/troll-zhao/tools/gopls/core/util/immutable"
...
func (s *Snapshot) load(ctx context.Context, allowNetwork bool, scopes ...loadScope) (err error) {
	...
	for _, scope := range scopes {
		...
		case fileLoadScope:
			...
			if len(scopes) > 1 {
				panic(fmt.Sprintf("core error: load called with multiple scopes when a file scope is present (file: %s)", uri))
			}
			fh := s.FindFile(uri)
		...
		for _, pkg := range pkgs {
			if pkg.ID == "command-line-arguments" {
				if standalonePkg != nil {
					return errorf("core error: go/packages returned multiple standalone packages")
				}
				standalonePkg = pkg
			} else if packagesinternal.GetForTest(pkg) == "" && !strings.HasSuffix(pkg.ID, ".test") {
				return errorf("core error: go/packages returned unexpected package %q for standalone file", pkg.ID)
			}
		}
		if standalonePkg == nil {
			return errorf("core error: go/packages failed to return non-test standalone package")
		}
