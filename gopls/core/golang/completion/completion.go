...
import (
	...
	goplsastutil "github.com/troll-zhao/tools/gopls/core/util/astutil"
...
type CompletionItem struct {
	...
	// Score is the core relevance score.
...
type completer struct {
	...
	// defined by x/tools/core/versions. Empty if unknown.
...
func (c *completer) populateImportCompletions(searchImport *ast.ImportSpec) error {
	...
	if strings.HasPrefix(importPath, `"`) && c.pos > searchImport.Path.Pos() {
		...
		if depth > 0 {
			...
			// path was "golang.or/x/to<>ols/core/", where <> is the cursor
