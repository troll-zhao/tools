...
func builtinDecl(ctx context.Context, snapshot *cache.Snapshot, obj types.Object) (*parsego.File, *ast.Ident, error) {
	...
	declaringIdent := func(file *ast.File, name string) (ast.Node, *ast.Ident, error) {
		...
		if astObj == nil {
			...
			return nil, nil, fmt.Errorf("core error: no object for %s", name)
		}
		decl, ok := astObj.Decl.(ast.Node)
		if !ok {
			return nil, nil, bug.Errorf("core error: no declaration for %s", obj.Name())
		}
		var ident *ast.Ident
		...
		default:
			return nil, nil, bug.Errorf("core error: unexpected decl type %T", decl)
		}
		if ident == nil {
			return nil, nil, bug.Errorf("core error: no declaring identifier for %s", obj.Name())
		}
