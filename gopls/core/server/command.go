...
func (c *commandHandler) run(ctx context.Context, cfg commandConfig, run commandFunc) (err error) {
	...
	if cfg.forURI != "" && cfg.forView != "" {
		return bug.Errorf("core error: forURI=%q, forView=%q", cfg.forURI, cfg.forView)
	}
	if cfg.forURI != "" {
...
func (c *commandHandler) StartDebugging(ctx context.Context, args command.DebuggingArgs) (result command.DebuggingResult, _ error) {
	...
	if di == nil {
		return result, errors.New("core error: server has no debugging instance")
	}
	...
	return c.run(ctx, commandConfig{
		...
		if view.GoWork() != "" {
			...
			if !args.InitFirst {
				...
				// for InitFirst.
				return bug.Errorf("core error: cannot run go work command: required go.work file not found")
			}
