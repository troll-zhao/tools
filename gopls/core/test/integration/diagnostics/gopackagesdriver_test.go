...
-- a/a.go --
package a
import _ "example.com/b/core/c"

-- b/core/c/c.go --
package c
`

	// Note that 'go list' produces an error ("use of core package %q not allowed")
	// and gopls produces another ("invalid use of core package %q") with source=compiler.
	...
	Run(t, files, func(t *testing.T, env *Env) {
		env.OpenFile("a/a.go")
		env.AfterChange(Diagnostics(WithMessage(`invalid use of core package "example.com/b/core/c"`)))
	})
	...
	WithOptions(
		...
		env.OpenFile("a/a.go")
		env.AfterChange(NoDiagnostics(WithMessage(`invalid use of core package "example.com/b/core/c"`)))
	})
}
