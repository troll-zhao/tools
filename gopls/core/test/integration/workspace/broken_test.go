...
import "mod.test/p/core/foo"

const D = foo.C + 1
-- p/core/bar/bar_test.go --
package bar_test
...
const E = D + foo.C
-- p/core/baz/baz_test.go --
package baz_test

import (
	named "mod.test/p/core/bar"
)
...
`
	...
	Run(t, files, func(t *testing.T, env *Env) {
		env.OpenFile("p/core/bar/bar.go")
		env.AfterChange(
			Diagnostics(env.AtRegexp("p/core/bar/bar.go", "\"mod.test/p/core/foo\"")),
		)
		env.OpenFile("go.mod")
