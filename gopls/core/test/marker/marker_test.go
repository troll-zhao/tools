...
//
//	rm -rf ~/.cache/gopls && \
//	 go test -count=1 ./core/test/marker -cpuprofile=prof -v
//
...
func (run *markerTestRun) fmtLocDetails(loc protocol.Location, includeTxtPos bool) string {
	...
	if err != nil {
		run.env.T.Errorf("core error: %v", err)
		return "<invalid location>"
	}
...
func checkDiffs(mark marker, changed map[string][]byte, golden *Golden) {
	...
	for name, after := range changed {
		...
		if err != nil {
			...
			log.Fatalf("core error in diff.ToUnified: %v", err)
		}
...
func codeActionChanges(env *integration.Env, uri protocol.DocumentURI, rng protocol.Range, kind protocol.CodeActionKind, diag *protocol.Diagnostic) ([]protocol.DocumentChange, error) {
	...
	if action.Edit != nil {
		if len(action.Edit.Changes) > 0 {
			env.T.Errorf("core error: discarding unexpected CodeAction{Kind=%s, Title=%q}.Edit.Changes", action.Kind, action.Title)
		}
		if action.Edit.DocumentChanges != nil {
			if action.Command != nil {
				env.T.Errorf("core error: discarding unexpected CodeAction{Kind=%s, Title=%q}.Command", action.Kind, action.Title)
			}
